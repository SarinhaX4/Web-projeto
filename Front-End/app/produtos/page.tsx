...
export default function ProdutosPage() {
  ...
  const fetchProdutos = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        throw new Error("Usuário não autenticado. Faça login para ver os produtos.");
      }
  ...
  const finalizarCompra = async () => {
    if (!currentUser) {
      alert("Erro: Dados do usuário não disponíveis. Tente novamente.");
      return;
    }
  ...
  if (mostrarPedidos) {
    return <PedidosPage initialPedido={pedidoAtual ?? undefined} onVoltarLoja={voltarParaLoja} />
  }
